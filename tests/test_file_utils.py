...
TEST_DIR = os.path.join(
    ...
    __file__ + "TestDir{}".format(int(random() * 100000)),
)
TEST_FILE = os.path.join(
    os.environ.get("TEMP", r"C:\Windows\Temp"),
    os.path.basename(__file__) + ".Test{}".format(int(random() * 100000)),
)
...
def test_get_ownership():
    ...
    assert (
        owner[0] == "TrustedInstaller"
    ), "{}} file owner bogus, should be TrustedInstaller since win7".format(
        local_windows_dir
    )
    assert (
        owner[1] == "NT SERVICE"
    ), "{} file owner should be of group NT SERVICE since Win7".format(
        local_windows_dir
    )
    assert owner[2] == 5, "{} file owner rights should be 5 since Win7".format(
        local_windows_dir
    )
...
def test_take_ownership():
    ...
    assert (
        new_owner_sid[0].__str__() == "PySID:S-1-5-18"
    ), "File should now be owned by system account"
    assert (
        new_owner != owner
    ), "Earlier owner and current owner should differ unless tests are run as system"
...
def test_get_paths_recursive_and_fix_permissions():
    ...
    if os.path.isdir(TEST_DIR):
        get_paths_recursive_and_fix_permissions(
            TEST_DIR, owner=whoami(), permissions=easy_permissions("RWX")
        )
        shutil.rmtree(TEST_DIR)
    os.makedirs(TEST_DIR)
    with open(
        os.path.join(TEST_DIR, os.path.basename(__file__) + ".tst"), "w"
    ) as file_handle:
    ...
    try:
        set_acls(
            ...
            user_list=[whoami()],
            owner=whoami(),
            permissions=easy_permissions("R"),
            inherit=False,
            inheritance=False,
        )
    except Exception as e:
    ...
    try:
        get_paths_recursive_and_fix_permissions(
            TEST_DIR, permissions=easy_permissions("F")
        )
    except Exception as e:
        assert (
            False
        ), "get_file_recursive_and_fix_permissions had an exception: {}".format(e)
