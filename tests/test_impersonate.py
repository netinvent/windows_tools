...
def test_ImpersonateWin32Sec():
    ...
    with ImpersonateWin32Sec(domain='.', username=TEST_USERNAME, password=TEST_PASSWORD):
        imp_current_user = win32api.GetUserName()
        print('Current impersonated user: ', current_user)
        assert current_user != imp_current_user, 'Current user and test user should be different'
        assert imp_current_user == TEST_USERNAME, 'Impersonated user should be test user'
    ...
    assert current_user != imp_current_user, 'Current user and test user should be different now'
