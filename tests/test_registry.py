...
def test_get_value():
    ...
    key = get_value(hive=HKEY_LOCAL_MACHINE, key=r'SOFTWARE\Microsoft\Windows NT\CurrentVersion',
                             value='ProductName', arch=KEY_WOW64_32KEY | KEY_WOW64_64KEY, last_modified=True)
    print('Product Name key: %s' % key['value'])
    print('Last modified: %s' % key['last_modified'])
    assert key['value'].startswith('Windows'), 'product_name should start with "Windows"'
...
def test_get_values():
    ...
    uninstall = get_values(hive=HKEY_LOCAL_MACHINE, key=r'SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall',
                           names=['DisplayName', 'Version'], arch=KEY_WOW64_32KEY | KEY_WOW64_64KEY,
                           last_modified=True)
    assert re.match(DATE_REGEX, uninstall[0][list(uninstall[0].keys())[0]]['last_modified']), 'Invalid date format'
...
def test_get_keys():
    ...
    assert len(uninstall_recursive) > 0, 'get_keys() should return at least one software to that could be uninstalled'
