...
def test_get_relevant_platform_info():
    ...
    assert isinstance(next(iter(platform_info.values())), dict), 'Virtualization check is not a sub-dictionnary'
    assert isinstance(platform_info['computersystem']['Manufacturer'], str), 'computer manufacturer should be found'
    assert isinstance(platform_info['baseboard']['Manufacturer'], str), 'baseboard manufacturer should be found'
    assert isinstance(platform_info['bios']['Manufacturer'], str), 'bios manufacturer should be found'
    assert isinstance(platform_info['diskdrive']['DeviceID'], str), 'computer should at least have one physical disk'
