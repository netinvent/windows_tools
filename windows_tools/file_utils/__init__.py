...
def get_ownership(path: str) -> Tuple[str, str, int]:
    ...
    """
    try:
        sec_descriptor = win32security.GetFileSecurity(path, win32security.OWNER_SECURITY_INFORMATION)
...
def take_ownership(path: str, owner=None, force: bool = True) -> bool:
    ...
    try:
        ...
        win32security.SetFileSecurity(path, win32security.OWNER_SECURITY_INFORMATION, sec_descriptor)
    except pywintypes.error as exc:
        # Let's raise OSError so we don't need to import pywintypes in parent module to catch the exception
...
def set_acls(path: str,
    ...
    try:
        sec_descriptor = win32security.GetNamedSecurityInfo(path, win32security.SE_FILE_OBJECT,
...
def get_files_recursive_and_fix_permissions(path: str, owner: object = None,
    ...
    def fix_perms(path: str) -> None:
        ...
        logger.error('Permission error on: {0}.'.format(path))
        try:
            set_acls(path, user_list=user_list, owner=owner, permissions=permissions, inheritance=False)
        except OSError:
